...
class TestStudyService:
    ...
    async def test_create_plan_creates_user_if_not_exists(self, study_service):
        with patch("services.study_service.storage") as mock_storage:
            ...
            mock_storage.create_plan = AsyncMock(return_value=Plan(1, 1, "Math"))
    ...
    @pytest.mark.asyncio
    async def test_add_task_creates_task_and_updates_deadline(self, study_service):
        with patch("services.study_service.storage") as mock_storage:
            mock_storage.create_task = AsyncMock(return_value=Task(1, 1, "Task 1", date(2025, 1, 1)))
            mock_storage.get_plan = AsyncMock(return_value=Plan(1, 1, "Math", date(2025, 1, 1)))
            mock_storage.update_plan_deadline = AsyncMock()

            result = await study_service.add_task(1, "Task 1", date(2025, 1, 1))

            assert result.title == "Task 1"

    @pytest.mark.asyncio
    async def test_update_plan_deadline_updates_if_new_is_later(self, study_service):
        with patch("services.study_service.storage") as mock_storage:
            mock_storage.get_plan = AsyncMock(return_value=Plan(1, 1, "Math", date(2025, 1, 1)))
            mock_storage.update_plan_deadline = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_update_plan_deadline_does_not_update_if_earlier(self, study_service):
        with patch("services.study_service.storage") as mock_storage:
            mock_storage.get_plan = AsyncMock(return_value=Plan(1, 1, "Math", date(2025, 12, 31)))
            mock_storage.update_plan_deadline = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_update_plan_deadline_updates_if_current_is_none(self, study_service):
        with patch("services.study_service.storage") as mock_storage:
            mock_storage.get_plan = AsyncMock(return_value=Plan(1, 1, "Math", None))
            mock_storage.update_plan_deadline = AsyncMock()
