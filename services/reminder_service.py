...
class ReminderService:
    ...
    async def set_reminder(self, task, plan, delta: timedelta, reminder_type: str):
        ...
        self.storage.save_reminder(task_id, user_id, title, deadline, reminder_type, int(delta.total_seconds()))
        ...
        for rem in self.reminders.values():
            ...
            if task_id not in task_map or days_left < task_map[task_id]['days_num']:
                ...
                task_map[task_id] = {
                    ...
                    'title': rem['title'],
                    'days_left': days_text,
                    'deadline': deadline_date.strftime('%d.%m.%Y') if hasattr(deadline_date, 'strftime') else str(deadline_date),
                    'days_num': days_left
                }
    ...
    async def check_and_send(self):
        ...
        for rem in due_reminders:
            ...
            type_messages = {
                'завтра': MessageTemplates.REMINDER_TOMORROW.format(time_text=time_text, deadline=deadline_str, title=title),
                'тиждень': MessageTemplates.REMINDER_WEEK.format(deadline=deadline_str, title=title),
                '24годин': MessageTemplates.REMINDER_24HOURS.format(deadline=deadline_str, title=title),
            }
            
            text = type_messages.get(reminder_type, MessageTemplates.REMINDER_DEFAULT.format(title=title, deadline=deadline_str))
