...
class TaskKeyboard:
    ...
    def task_detail(task: Union[dict, object], plan_subject: str):
        ...
        if is_done:
            keyboard.append([InlineKeyboardButton("↩️ Позначити як невиконане", callback_data=f"undone_{task_id}")])
        else:
            keyboard.append([InlineKeyboardButton("✅ Позначити як виконане", callback_data=f"done_{task_id}")])
        
        keyboard.append([InlineKeyboardButton("🗑️ Видалити завдання", callback_data=f"delete_task_{task_id}")])
