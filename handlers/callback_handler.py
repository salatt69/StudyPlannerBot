...
class CallbackHandler:
    ...
    async def _back_to_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.edit_message_text(MessageTemplates.MAIN_MENU, reply_markup=MainMenuKeyboard.build())
    ...
    async def _handle_calendar_nav(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
        query = update.callback_query
        parts = data.split("_")
        year = int(parts[-2])
        month = int(parts[-1])
        ...
        await query.edit_message_text(
            ...
            reply_markup=CalendarKeyboard.build(year, month, context.user_data.get("cal_prefix", "date_"))
        )
    ...
    async def _show_plans_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        plans_data = [
            {"plan_id": p.plan_id, "subject": p.subject, "deadline": p.deadline}
            for p in plans
        ]
        await query.edit_message_text(
    ...
    async def _show_plan_detail(self, update: Update, context: ContextTypes.DEFAULT_TYPE, plan_id: int):
        query = update.callback_query
        user_id = update.effective_user.id
        plan = self.study_service.get_plan_by_id(plan_id)
        
        if not plan:
            await query.edit_message_text(MessageTemplates.PLAN_NOT_FOUND, reply_markup=PlanKeyboard.back("back_to_plans"))
            return
        
        if plan.user_id != user_id:
            await query.edit_message_text(MessageTemplates.PLAN_NOT_FOUND, reply_markup=PlanKeyboard.back("back_to_menu"))
            return
        ...
        if not tasks:
            await query.edit_message_text(
                MessageTemplates.PLAN_DETAIL_NO_TASKS.format(subject=plan.subject, deadline=deadline_text),
    ...
    async def _show_task_detail(self, update: Update, context: ContextTypes.DEFAULT_TYPE, task_id: int):
        query = update.callback_query
        user_id = update.effective_user.id
    ...
    async def _mark_task_done(self, update: Update, context: ContextTypes.DEFAULT_TYPE, task_id: int):
        query = update.callback_query
    ...
    async def _mark_task_undone(self, update: Update, context: ContextTypes.DEFAULT_TYPE, task_id: int):
        query = update.callback_query
    ...
    async def _show_plans_for_task(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        plans_data = [
            {"plan_id": p.plan_id, "subject": p.subject}
            for p in plans
        ]
        await query.edit_message_text(
            MessageTemplates.SELECT_PLAN,
    ...
    async def _show_task_title_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE, plan_id: int):
        query = update.callback_query
    ...
    async def _show_task_deadline_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE, plan_id: int):
        query = update.callback_query
    ...
    async def _select_date(self, update: Update, context: ContextTypes.DEFAULT_TYPE, year: int, month: int, day: int):
        query = update.callback_query
